...
class StoreModuleSite extends WeModuleSite {
		...
		if($params['result'] == 'success' && $params['from'] == 'notify') {
			...
			if(!empty($order)) {
				...
				if (in_array($goods['type'], array(STORE_TYPE_ACCOUNT_RENEW, STORE_TYPE_WXAPP_RENEW))) {
					...
					$store_create_account_info = table('store_create_account')->getByUniacid($order[$account_type]);
	...
	public function doWebPayments() {
		...
		if ($payments_list) {
			$goods = table('store_goods')->where('id', array_column($payments_list, 'goodsid'))->getall('id');
